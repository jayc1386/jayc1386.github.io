...
* It contains measurements (in centimeters) of the variables of sepal length & width and petal length & width for `r nrow(iris)` flowers of each of three species of iris.
* The species are `r paste0("iris ", unique(iris$Species)[1])`, `r paste0("iris ", unique(iris$Species)[2])`, and `r paste0("iris ", unique(iris$Species)[3])`.
...
rpModelFit <- train(Species ~., data = training, method = "rpart")
rp <- data.frame(actual = testing$Species, predicted = predict(rpModelFit, newdata = testing))
rpAcc <- paste0(as.character(100*sum(rp$actual == rp$predicted)/nrow(rp)),"%")

rfModelFit <- train(Species ~., data = training, method = "rf")
rf <- data.frame(actual = testing$Species, predicted = predict(rfModelFit, newdata = testing))
rfAcc <- paste0(as.character(100*sum(rf$actual == rf$predicted)/nrow(rf)),"%")
```
...
* The accuracy of the rpart algorithm for this dataset under the set seed is `r paste0(as.character(round(99/108,4)*100),"%")`.
* The accuracy of the rf algorithm for this dataset under the set seed is `r paste0(as.character(round(8/9,4)*100),"%")`.
